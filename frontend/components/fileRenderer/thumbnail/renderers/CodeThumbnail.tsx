...
export default async function CodeThumbnail({ code }: CodeThumbnailProps) {
    ...
    const primaryLang =
        LANGUAGES.find((l) => l.value === primaryFile.language)?.value ?? "javascript";
    ...
    return (
        ...
        >
            ...
            <p className="text-text-secondary">{code.description}</p>
            <div>
                <div className="flex flex-row gap-xs">
                    {code.codeFiles.map((codeFile, index) => {
                        const lang = LANGUAGES.find((l) => l.value === codeFile.language);
                        const Icon = lang?.icon;
                        return Icon ? (
                            <Icon key={(codeFile.language, index)} />
                        ) : (
                            <span key={(codeFile.language, index)}>{codeFile.language}</span>
                        );
                    })}
                </div>
            </div>
            <div className="flex flex-col gap-1">
                <div className="flex flex-row justify-between text-xs">
                    <div>
                        <p className="text-text-secondary">{primaryFile.title}</p>
                    </div>
                    {code.codeFiles.length > 1 && (
                        <div className="flex flex-row items-center text-xs">
                            <File size={16} />
                            <p>+{code.codeFiles.length - 1} files</p>
                        </div>
                    )}
                </div>
                <div
