...
export async function uploadFilesToServer(files: File[], fileRole = "RESOURCE_FILE") {
    ...
    formData.append(
        "map",
        JSON.stringify(Object.fromEntries(files.map((_, i) => [i.toString(), [`variables.files.${i}`]])))
    );
